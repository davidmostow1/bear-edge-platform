...
-- Research-only evidence plumbing. This migration cannot by itself authorize a wager.
...
create table if not exists public.model_market_snapshots (
  ...
  content_digest text not null check (content_digest ~ '^[a-f0-9]{64}$'),
  created_at timestamptz not null default now(),
  check (team_a_yes_bid is null or (team_a_yes_bid between 0 and 1)),
  check (team_a_yes_ask is null or (team_a_yes_ask between 0 and 1)),
  check (team_b_yes_bid is null or (team_b_yes_bid between 0 and 1)),
  check (team_b_yes_ask is null or (team_b_yes_ask between 0 and 1)),
...
create table if not exists public.model_probability_intervals (
  ...
  calibrated_probability_a double precision not null check (calibrated_probability_a between 0 and 1),
  uncertainty_low_a double precision not null check (uncertainty_low_a between 0 and 1),
  uncertainty_high_a double precision not null check (uncertainty_high_a between 0 and 1),
  method text not null,
  confidence_level double precision not null check (confidence_level > 0 and confidence_level < 1),
  generated_at timestamptz not null,
  content_digest text not null check (content_digest ~ '^[a-f0-9]{64}$'),
...
insert into public.model_promotion_policies (policy_version, policy_digest, registered_at, policy)
values (
  '1.2.0',
  'ee24c6dc23dfa5cf9384d0bb595193b41903024e4e5959e54694800a2cb4226a',
  '2026-07-23T03:13:15.000Z',
  '{"minimumSettledPredictions":500,"minimumDistinctEvents":100,"minimumBucketObservations":100,"minimumSettlementCoverage":0.95,"maximumExpectedCalibrationError":0.03,"minimumCalibrationSlope":0.8,"maximumCalibrationSlope":1.2,"maximumAbsoluteCalibrationIntercept":0.05,"requireNoMaterialBaselineDegradation":true,"requireNonNegativeClosingLineValueInterval":true,"reliabilityBucketBoundaries":[0,0.2,0.4,0.6,0.8,1],"requiredBaseline":{"baselineId":"no_vig_market","method":"two_way_proportional_normalization","methodVersion":"1.0.0"},"requiredSplitMethod":"event_atomic_prediction_interval_blocks","requiredUncertaintyMethod":"event_cluster_percentile_bootstrap","minimumBootstrapResamples":2000,"minimumConfidenceLevel":0.95}'::jsonb
)
...
begin
  foreach table_name in array array[
    'model_prediction_context','model_prediction_status_events','model_event_outcomes',
    'model_market_snapshots','model_promotion_policies','model_calibrator_artifacts',
    'model_probability_intervals','model_promotion_evaluations'
  ] loop
    ...
    execute format(
      ...
      'sbkp_append_only_' || table_name, table_name
    );
  end loop;
end;
$$;
...
select
  p.id,p.model_id,p.canonical_event_id,p.market_family,p.team_a,p.team_b,p.best_of,
  p.raw_probability_a,p.calibrated_probability_a,p.uncertainty_low_a,p.uncertainty_high_a,
  p.generated_at,p.evidence_cutoff_at,p.input_snapshot,p.output_snapshot,p.content_digest,
  c.scheduled_start_at,c.event_identity_status,c.roster_status,c.patch_status,
  c.source_snapshot,c.source_digest,s.effective_at as valid_effective_at
...
with latest_outcomes as (
  select * from (
    select o.*,
      row_number() over (partition by o.canonical_event_id order by o.created_at desc,o.resolved_at desc,o.id desc) as r
    from public.model_event_outcomes o
  ) x where r=1
), latest_decision as (
  select * from (
    select s.*,
      row_number() over (partition by s.prediction_id order by s.captured_at desc,s.created_at desc,s.id desc) as r
  ...
  select * from (
    select s.*,
      row_number() over (partition by s.canonical_event_id order by s.captured_at desc,s.created_at desc,s.id desc) as r
    from public.model_market_snapshots s
  ...
  select * from (
    select i.*,
      row_number() over (partition by i.prediction_id order by i.generated_at desc,i.created_at desc,i.id desc) as r
    from public.model_probability_intervals i
  ) x where r=1
)
select p.*,
  o.event_status,o.winner_team,o.resolved_at,
  case when o.event_status='FINAL' and o.winner_team=p.team_a then 1
       when o.event_status='FINAL' and o.winner_team=p.team_b then 0 else null end as outcome_a,
  d.team_a_yes_ask as decision_team_a_yes_ask,d.team_b_yes_ask as decision_team_b_yes_ask,
  case when d.team_a_yes_ask is not null and d.team_b_yes_ask is not null and d.team_a_yes_ask+d.team_b_yes_ask>0
       then d.team_a_yes_ask/(d.team_a_yes_ask+d.team_b_yes_ask) else null end as decision_no_vig_probability_a,
  c.team_a_yes_ask as closing_team_a_yes_ask,c.team_b_yes_ask as closing_team_b_yes_ask,
  case when c.team_a_yes_ask is not null and c.team_b_yes_ask is not null and c.team_a_yes_ask+c.team_b_yes_ask>0
       then c.team_a_yes_ask/(c.team_a_yes_ask+c.team_b_yes_ask) else null end as closing_no_vig_probability_a,
...
begin
  if gpr_a::text in ('NaN','Infinity','-Infinity') or gpr_b::text in ('NaN','Infinity','-Infinity') then
  ...
  if best_of=1 then return p;
  elsif best_of=3 then return p*p*(3.0-2.0*p);
  elsif best_of=5 then return p*p*p*(10.0-15.0*p+6.0*p*p);
  end if;
...
begin
  select * into model_row from public.model_registry where model_id=p_model_id for update;
  if not found or model_row.model_status<>'SHADOW' or model_row.bet_authority=true then return false; end if;

  select * into policy_row from public.model_promotion_policies order by registered_at desc,created_at desc limit 1;
  if not found then return false; end if;

  select * into evaluation_row
  from public.model_promotion_evaluations
  where model_id=p_model_id and policy_id=policy_row.id and passed=true
  order by evaluated_at desc,created_at desc limit 1;
  if not found or jsonb_typeof(evaluation_row.checks)<>'array' or jsonb_array_length(evaluation_row.checks)=0 then return false; end if;
  ...
  where id=evaluation_row.calibration_run_id and model_id=p_model_id
    and run_type='PROSPECTIVE' and status='PASSES_PROMOTION';
  if not found then return false; end if;
  ...
  if calibration_row.sample_size < (policy_row.policy->>'minimumSettledPredictions')::integer
     ...
     or abs(calibration_row.calibration_intercept) > (policy_row.policy->>'maximumAbsoluteCalibrationIntercept')::double precision
  then return false; end if;

  select * into artifact_row
  from public.model_calibrator_artifacts
  where id=evaluation_row.calibrator_artifact_id and model_id=p_model_id
    and calibration_run_id=calibration_row.id and status='VERIFIED';
  if not found
     ...
     or artifact_row.confidence_level < (policy_row.policy->>'minimumConfidenceLevel')::double precision
  then return false; end if;

  update public.model_registry
  set model_status='VALIDATED',prospective_calibration_status='VERIFIED',uncertainty_status='VERIFIED',
      bet_authority=true,promoted_at=coalesce(promoted_at,now())
