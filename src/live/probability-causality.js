...
function normalizeBet(input, index) {
  ...
  const marketImpliedProbability = input.marketImpliedProbability === undefined
...
function simulateBetCard(input = {}) {
  const bets = (Array.isArray(input.bets) ? input.bets : []).map(normalizeBet);
  ...
  return {
    ...
    assumptions: {
      ...
      stakeSizing: "Stake and payout math use the visible screenshot bankroll and American odds supplied in the input."
    },
    trials
  };
}
